...
class TodoDbSql():
    ...
    def reconnect(self):
        try:
            cConnection= pymysql.connect(host=self.settings.addr, port=3306, user=self.settings.login, passwd=self.settings.passw, db=self.settings.scheme, use_unicode=True, charset="utf8")
